          ...
          <div className="max-w-[83.75rem] mx-auto px-6 grid lg:grid-cols-[1.1fr_1fr] gap-16 min-h-[70vh] items-center relative z-10">
            <Reveal className="max-w-[35rem]">
              ...
              <p className="text-[clamp(1rem,1.2vw,1.125rem)] mb-10 text-muted-foreground max-w-[28rem]">
                Timbre AI provides developers with an institutional-grade synthesis engine and
              ...
              <div className="grid grid-cols-3 gap-6 font-mono-custom text-[0.6875rem] text-muted-foreground border-t border-border pt-6">
                <div>
                  <div className="text-foreground text-lg mb-1">
                    <CountingNumber value={2.47} />B
                  </div>
                  Characters Generated
                </div>
                <div>
                  <div className="text-foreground text-lg mb-1">
                    <CountingNumber value={34} />
                  </div>
                  Languages Supported
                </div>
                <div>
                  <div className="text-primary text-lg mb-1">
                    <CountingNumber value={99.9} format="percent" />%
                  </div>
                  Synthesis Quality
                </div>
              </div>
          ...
          <div className="max-w-[83.75rem] mx-auto px-6 grid md:grid-cols-[1.2fr_1fr] gap-24 items-start relative z-10">
            <Reveal className="max-w-[38rem]">
              ...
              <p className="text-muted-foreground text-[clamp(1rem,1.1vw,1.125rem)]">
                ...
                robotic, monotone models. Creators endure poor engagement that could be bypassed
                with expressive, AI-cloned identity in real-time.
              </p>
            </Reveal>
            ...
            <Reveal className="grid grid-cols-2 gap-y-16 gap-x-8">
              <div className="relative border-l border-border pl-6">
                ...
                <div className="font-mono-custom text-4xl text-foreground mb-2 tracking-tight">
                  <CountingNumber value={7.2} />
                  <span className="text-muted-foreground text-lg ml-1">ms</span>
                </div>
              ...
              <div className="relative border-l border-border pl-6">
                <div className="absolute top-0 left-[-1px] w-[2px] h-4 bg-primary"></div>
                <div className="font-mono-custom text-4xl text-foreground mb-2 tracking-tight">
                  $<CountingNumber value={340} format="k" />
                  <span className="text-muted-foreground text-lg ml-1">K</span>
                </div>
                <div className="text-[0.625rem] uppercase tracking-wider text-muted-foreground">
                  ANNUAL ASSET LOSS
                </div>
            ...
            <div className="grid lg:grid-cols-[2fr_1fr] gap-6">
              ...
              <Reveal className="glass-panel rounded-sm relative overflow-hidden group min-h-[450px]">
                ...
                <div className="absolute inset-0 flex items-center justify-center z-10 pointer-events-none">
                  ...
                  >
                    <path
                      d="M150 100 L300 200 L450 120 M300 200 L250 320 L400 300"
                      strokeWidth="1.5"
                      strokeDasharray="4 4"
                    ...
                    <g>
                      ...
                      <circle
                        ...
                        strokeWidth="1"
                        strokeOpacity="0.5"
                        className="animate-ping stroke-primary"
                      />
                    </g>
                  </svg>
                </div>
              ...
              <div className="grid grid-rows-2 gap-6">
                <Reveal className="glass-panel rounded-sm p-8 flex flex-col justify-center relative overflow-hidden">
                  <Icon
                    ...
                    className="text-muted-foreground text-2xl mb-4"
                  />
                  <h3 className="text-foreground text-lg tracking-tight mb-2">
                    High-Fidelity Synthesis
                  </h3>
                  <div className="font-mono-custom text-4xl text-primary mb-2 leading-none tracking-tighter">
                    <CountingNumber value={99.9} format="percent" />%
                  </div>
                  <p className="text-[0.75rem] text-muted-foreground">
                    Studio-quality audio output with natural inflection and emotive resonance.
                  </p>
                </Reveal>
                <Reveal className="glass-panel rounded-sm p-8 flex flex-col justify-center relative overflow-hidden">
                  <Icon icon="solar:wave-linear" className="text-muted-foreground text-2xl mb-4" />
                  <h3 className="text-foreground text-lg tracking-tight mb-2">
                    Vocal Topology Engine
                  </h3>
                  <p className="text-[0.75rem] text-muted-foreground">
                    Autonomous cloning protocols map 1,000+ unique vocal characteristics from 30
                    seconds of audio.
                  </p>
                </Reveal>
              </div>
                  ...
                  <div className="flex flex-col gap-2">
                    {[65, 80, 95, 60, 70].map((w, i) => (
                      <div key={i} className="h-6 flex justify-end relative group">
                        <div
                          className="bg-primary/20 absolute right-0 h-full border-l border-primary transition-all duration-1000 delay-[500ms]"
                          style={{ width: `${w}%` }}
                        ></div>
                        <span className="relative z-10 font-mono text-[0.625rem] leading-6 pr-3 text-foreground">
                          {100 - i * 10}k | {(2.68 - i * 0.02).toFixed(2)}x
                        </span>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="flex flex-col gap-2">
                    {[75, 60, 90, 65, 85].map((w, i) => (
                      <div key={i} className="h-6 flex justify-start relative group">
                        <div
                          className="bg-foreground/10 absolute left-0 h-full border-r border-foreground/50 transition-all duration-1000 delay-[500ms]"
                          style={{ width: `${w}%` }}
                        ></div>
                        <span className="relative z-10 font-mono text-[0.625rem] leading-6 pl-3 text-foreground">
                          {(2.71 + i * 0.04).toFixed(2)}x | {85 + i * 20}k
                        </span>
                      </div>
                    ))}
                  </div>
          ...
          <div className="max-w-[83.75rem] mx-auto px-6 grid lg:grid-cols-[1.1fr_1fr] gap-20">
            <Reveal>
              ...
              <p className="text-muted-foreground mb-12 max-w-[28rem] text-[1.0625rem]">
                Timbre AI restricts API access to a hermetic developer cohort. We process exactly 12
                institutional onboarding protocols per quarter.
              </p>
              ...
              <div className="glass-panel p-6 rounded-sm border-l-2 border-l-primary max-w-sm">
                ...
                <ul className="font-mono-custom text-[0.75rem] text-foreground space-y-2">
                  <li className="flex justify-between">
                    <span>ACTIVE_CYCLE:</span>{' '}
                    <span className="text-muted-foreground">Q1 2026</span>
                  </li>
                  <li className="flex justify-between">
                    <span>CAPACITY_REMAINING:</span>{' '}
                    <span className="text-muted-foreground">04 SLOTS</span>
                  </li>
                  <li className="flex justify-between">
                    <span>PREREQUISITE:</span>{' '}
                    <span className="text-muted-foreground">API_CLEARANCE</span>
                  </li>
                </ul>
              </div>
            </Reveal>
