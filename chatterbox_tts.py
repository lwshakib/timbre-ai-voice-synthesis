...
# Load local .env for deployment context (bucket names, endpoints)
load_dotenv()

# Setup Storage Protocol
# This script uses standard S3-compatible environment variables:
# AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY, AWS_ENDPOINT, AWS_S3_BUCKET_NAME, AWS_REGION

# Mount Configuration
MOUNT_PATH = "/storage"

def get_mount():
    """Returns the appropriate Modal mount using CloudBucketMount for S3 compat."""
    bucket_name = os.environ.get("AWS_S3_BUCKET_NAME", "timbre-ai-voices")
    endpoint_url = os.environ.get("AWS_ENDPOINT", "")
    
    kwargs = {
        "secret": modal.Secret.from_name("s3-storage"),
        "read_only": True,
    }
    
    if endpoint_url:
        kwargs["bucket_endpoint_url"] = endpoint_url

    return modal.CloudBucketMount(bucket_name, **kwargs)
...
@app.cls(
    ...
    secrets=[
        ...
        modal.Secret.from_name("s3-storage"), # Unified secret name
    ],
    volumes={MOUNT_PATH: get_mount()},
)
...
class Chatterbox:
    ...
    def serve(self):
        ...
        def generate_speech(request: TTSRequest):
            voice_path = Path(MOUNT_PATH) / request.voice_key
...
def test(
    ...
    audio_prompt_path = f"{MOUNT_PATH}/{voice_key}"
